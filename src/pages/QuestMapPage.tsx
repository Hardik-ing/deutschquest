    ...
    <div className="grid gap-8">
      <Card className="rounded-[36px] p-8">
        <div className="space-y-4">
          <p className="text-sm uppercase tracking-[0.3em] text-premiumCrimson font-bold">Quest Map</p>
        ...
        <div className="mt-8 grid gap-4 lg:grid-cols-2">
          {questList.map((quest, index) => {
            ...
            return (
              <motion.div
                key={quest.id}
                className={`rounded-[24px] border p-6 transition ${isCompleted ? 'border-green-500/40 bg-green-500/10' : isUnlocked ? 'border-premiumNavy bg-premiumNavy/60' : 'border-slate-800 bg-black/60 opacity-60'}`}
              ...
              >
                ...
                <div className="mt-4 grid gap-3 sm:grid-cols-[1fr,auto]">
                  <Link
                    ...
                    className={`rounded-full px-4 py-2 text-sm font-semibold transition text-center ${isUnlocked ? 'bg-premiumCrimson text-white hover:bg-red-700' : 'cursor-not-allowed bg-slate-800 text-slate-500'}`}
                  >
                    Open Lesson
                  </Link>
                  <button
                    ...
                    className={`rounded-full px-4 py-2 text-sm font-semibold transition ${isCompleted ? 'bg-green-600 text-white' : 'bg-slate-800 text-slate-200 hover:bg-slate-700'} ${!isUnlocked ? 'cursor-not-allowed opacity-50' : ''}`}
                  >
        ...
        <div className="mt-8 rounded-[28px] border border-white/10 bg-black/50 p-6">
